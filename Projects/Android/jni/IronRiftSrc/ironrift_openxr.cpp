...
static void PollEvents(AndroidXRRuntime &xr) {
    ...
    while (xrPollEvent(xr.instance, &event) == XR_SUCCESS) {
        if (event.type == XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED) { auto *changed = reinterpret_cast<XrEventDataSessionStateChanged *>(&event); xr.state = changed->state; XR_LOG("session state=%d running=%d", (int)xr.state, xr.running ? 1 : 0); if (xr.state == XR_SESSION_STATE_READY && !xr.running) { XrSessionBeginInfo begin{}; begin.type = XR_TYPE_SESSION_BEGIN_INFO; begin.primaryViewConfigurationType = XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO; if (XR_Ok(xrBeginSession(xr.session, &begin), "xrBeginSession")) xr.running = true; } if ((xr.state == XR_SESSION_STATE_STOPPING || xr.state == XR_SESSION_STATE_EXITING || xr.state == XR_SESSION_STATE_LOSS_PENDING) && xr.running) { xrEndSession(xr.session); xr.running = false; } }
...
static void UpdateActions(AndroidXRRuntime &xr) {
    if (!xr.action_set) return; XrActiveActionSet active{}; active.actionSet = xr.action_set; XrActionsSyncInfo sync{}; sync.type = XR_TYPE_ACTIONS_SYNC_INFO; sync.countActiveActionSets = 1; sync.activeActionSets = &active; if (xrSyncActions(xr.session, &sync) != XR_SUCCESS) return; iw_xr_action_snapshot_t actions{};
